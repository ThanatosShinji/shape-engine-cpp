          ...
          for (int ix = 0; ix < KX; ix++) {
            ...
            if (srcx >= 0 && srcx < w) {
              for (int icn = 0; icn < ic; icn++) {
                ...
                auto wval =
                    wptr[ioc * KY * KX * ic + iy * KX * ic + ix * ic + icn];
                tmp += xval * wval;
              }
            }
          }
      ...
      for (int ii = 0; ii < mIC; ii++) {
        for (int iky = 0; iky < mKernel[0]; iky++) {
          for (int ikx = 0; ikx < mKernel[1]; ikx++) {
            tmp[io * mKernel[0] * mKernel[1] * mIC + iky * mKernel[1] * mIC +
                ikx * mIC + ii] =
...
public:
  ...
  sycl::event forward(std::vector<void *> &mInputs,
                      ...
                      FckDNNL &_stream) override {
    auto q = _stream.q;
    auto eng = *_stream.eng;
    auto stream = *_stream.stream;
    auto x = (float *)mInputs[0];
    auto xshape = mIShapes[0];
    M = mTransA[0] ? xshape.ptr[1] : xshape.ptr[0];
    auto batch = mTransA[0] ? xshape.ptr[0] : xshape.ptr[1];
    auto y = (float *)mOutputs[0];
    auto yshape = mOShapes[0];
    auto xsize = shape_size(xshape);
    auto ysize = shape_size(yshape);
    ...
    auto coltransA =
        !mTransB[0] ? oneapi::mkl::transpose::N : oneapi::mkl::transpose::T;
    auto coltransB =
        !mTransA[0] ? oneapi::mkl::transpose::N : oneapi::mkl::transpose::T;
    ...
    auto _N = N;
    auto _M = M;
    oneapi::mkl::blas::gemm(_stream, coltransA, coltransB, colM, colN, colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::any);
    ...
    dnnl::memory::dims bias_dims = {1, 1, N};
    dnnl::memory::dims dst_dims = {batch, M, N};
    auto src_md = dnnl::memory::desc(src_dims, dt::f32, tag::abc);
    auto weights_md = dnnl::memory::desc(weights_dims, dt::f32, tag::abc);
    auto bias_md = dnnl::memory::desc(bias_dims, dt::f32, tag::abc);
    auto dst_md = dnnl::memory::desc(dst_dims, dt::f32, tag::abc);
    dnnl::memory src_m = dnnl::sycl_interop::make_memory(
        src_md, eng, dnnl::sycl_interop::memory_kind::usm, x);
    dnnl::memory weights_m = dnnl::sycl_interop::make_memory(
        weights_md, eng, dnnl::sycl_interop::memory_kind::usm, mDWeight.data());
    dnnl::memory bias_m = dnnl::sycl_interop::make_memory(
        bias_md, eng, dnnl::sycl_interop::memory_kind::usm, mDBias.data());
    dnnl::memory dst_m = dnnl::sycl_interop::make_memory(
    ...
    matmul_args.insert({DNNL_ARG_SRC, src_m});
    matmul_args.insert({DNNL_ARG_WEIGHTS, weights_m});
    matmul_args.insert({DNNL_ARG_BIAS, bias_m});
    matmul_args.insert({DNNL_ARG_DST, dst_m});
    auto e = sycl_interop::execute(matmul_prim, stream, matmul_args);
    return e;
#endif
#endif
  ...
  virtual void setweights(std::vector<onnx_tool::Tensor> &tensors,
    ...
    auto &weight = tensors[0];
    auto rawptr = (float *)weight.mRawptr;
    auto newsize = weight.mShape[0] * weight.mShape[1];
    ...
    _q.memcpy(mDWeight.data(), mWeights.data(), newsize * sizeof(float)).wait();
    if (tensors.size() > 1) {
      auto &bias = tensors[1];
      mBias.resize(N);
      mDBias.resize(N, _q);
