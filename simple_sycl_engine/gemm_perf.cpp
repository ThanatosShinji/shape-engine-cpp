...
void print_test_case(dt type, gemm_dims_t dims) {
  ...
  if (dims.m == dims.n && dims.m == dims.k)
    ...
    std::cout << " m = " << dims.m << ", n = " << dims.n << ", k = " << dims.k;
  std::cout << "] " << std::flush;
}
...
double run_case(engine::kind engine_kind, dt type, gemm_dims_t dims,
  ...
  auto coltransA = oneapi::mkl::transpose::N;
  auto coltransB = oneapi::mkl::transpose::N;
  auto colLDA = coltransA == oneapi::mkl::transpose::N ? colM : colK;
  auto colLDB = coltransB == oneapi::mkl::transpose::N ? colK : colN;
  auto colLDC = colM;
  auto sycl_queue = dnnl::sycl_interop::get_queue(dnnl::stream(engine));
  start = std::chrono::steady_clock::now();
  if (type == dt::bf16) {
    auto colA = (oneapi::mkl::bfloat16 *)a_mem.get_data_handle();
    auto colB = (oneapi::mkl::bfloat16 *)b_mem.get_data_handle();
    auto colC = (oneapi::mkl::bfloat16 *)c_mem.get_data_handle();
    start_first = std::chrono::steady_clock::now();
    oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN, colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::unset);
    sycl_queue.wait();
    end_first = std::chrono::steady_clock::now();
    dur_first = end_first - start_first;

    for (int i = 0; i <= runs; i++)
      oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN,
                              colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::unset);
   
  } else if (type == dt::f16) {
    auto colA = (sycl::half *)a_mem.get_data_handle();
    auto colB = (sycl::half *)b_mem.get_data_handle();
    auto colC = (sycl::half *)c_mem.get_data_handle();
    start_first = std::chrono::steady_clock::now();
    oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN, colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::unset);
    ...
    for (int i = 0; i <= runs; i++)
      oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN,
                              colK, 1.f, colA, colLDA, colB, colLDB, 0.f, colC,
                              colLDC, oneapi::mkl::blas::compute_mode::unset);
  } else if (type == dt::f32) {
    auto colA = (float *)a_mem.get_data_handle();
    auto colB = (float *)b_mem.get_data_handle();
    auto colC = (float *)c_mem.get_data_handle();
    start_first = std::chrono::steady_clock::now();
    oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN, colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::unset);
    ...
    auto colA = (int8_t *)a_mem.get_data_handle();
    auto colB = (int8_t *)b_mem.get_data_handle();
    auto colC = (int32_t *)c_mem.get_data_handle();
    start_first = std::chrono::steady_clock::now();
    oneapi::mkl::blas::gemm(sycl_queue, coltransA, coltransB, colM, colN, colK,
                            1.f, colA, colLDA, colB, colLDB, 0.f, colC, colLDC,
                            oneapi::mkl::blas::compute_mode::unset);
  ...
  try {
    if (dims.m * dims.n != 0) {
      ...
      while (mnk < max_mnk) {
        ...
        double mnk_expand =
            std::exp2(std::round(std::log2(nruns_est / min_runs) / 3.));
        if (mnk_expand <= 1)
          break;
        mnk = static_cast<int>(std::min<double>(max_mnk, mnk * mnk_expand));
      }
...
int main(int argc, char **argv) {
  return handle_example_errors(matmul_perf, parse_engine_kind(argc, argv, 3),
                               argc, argv);
}
