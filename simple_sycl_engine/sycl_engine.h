...
namespace sycl_engine
{
	...
	public:
		...
		float elapsed_time()
		{
			...
			const auto startKernExecutionTimePoint =
				mEvent.get_profiling_info<sycl::info::event_profiling::command_submit>();
	...
	public:
		...
		virtual sycl::event forward(std::vector<void*>& mInputs, std::vector<tensor_shape_t>& mIShapes, std::vector<void*>& mOutputs, std::vector<tensor_shape_t>& mOShapes, sycl::queue& _stream) = 0;

		virtual void setweights(std::vector<onnx_tool::Tensor>& tensors,sycl::queue& _q) = 0;
...
#define REGISTER_LAYER(type) static LayerRegistry<LayerCreatorT<type>> _LayerRegistry##type(#type);
	...
	public:
		...
		void forward(DynamicBindings* _bindings)
		{
			...
			for (int i = 0; i < mLayers.size(); i++)
			{
				_bindings->mEvents[i].mEvent = mLayers[i]->forward(_bindings->mLayerInPtr[i], _bindings->mLayerInShape[i], _bindings->mLayerOutPtr[i], _bindings->mLayerOutShape[i], _bindings->mQueue);
			}
		}
		...
		void save_proflie(const char* _file, DynamicBindings* _bindings)
		{
			...
			for (int i = 0; i < mLayers.size(); i++)
			{
				fprintf(fp, "%s,%s,%f\n", mLayers[i]->mName.c_str(), mLayers[i]->mOp.c_str()
					, _bindings->mEvents[i].execute_time());
			}
	...
	public:
		...
		DynamicBindings* createDynamicBindings(const variable_pairs_t& max_shape, bool profile = false)
		{
