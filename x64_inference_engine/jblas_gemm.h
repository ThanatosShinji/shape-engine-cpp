...
enum JBLAS_COMP_TYPE {
  JBLAS_COMP_UNDEF = 0, // infer from activation dtype
  JBLAS_COMP_F32 = 1,   // input f32, accumulator f32
  JBLAS_COMP_BF16 = 2,  // input bf16, accumulator f32
  JBLAS_COMP_F16 = 3,   // input f16, accumulator f16
  JBLAS_COMP_INT8 = 4,  // input int8, accumulator int32 (fp32 for intra block) };
};
...
bool JblasGemmPackB(void* PackedBuf, const int8_t* QData, const float* Scales, const int8_t* Zp, size_t N, size_t K,
                    ...
                    JBLAS_COMP_TYPE CompType, int *shuffle_indice,
                    void *ThreadPool);
