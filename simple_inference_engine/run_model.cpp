...
void inference_resnet18() {
  ...
  auto outputidx = ctx.mDynamicIndexMap["resnetv15_dense0_fwd"]; // output
                                                                 // tensor
  ...
  dbindings->reshape(
      {{"w", 112}, {"h", 122}}); // all shapes in this bindings will be updated
  printf("\n1x3x112x122\n");
  runtime->forward(dbindings);
  osize = std::accumulate(out_shape.ptr, out_shape.ptr + out_shape.n, 1,
                          std::multiplies<int>());
  for (int i = 0; i < osize; i++) {
    printf("%f ", outputptr[i]);
  }
