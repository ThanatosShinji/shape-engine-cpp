...
void load_shape_engine()
{
	...
	for (int i = 0; i < engine.mDynamicTensors.size(); i++)
	{
		auto tname = engine.mDynamicTensors[i].c_str();
		auto nlen = engine.get_tensor_shape_len(tname);
		printf("Tensor: %s\n", tname);
		printf("    ");
		auto shapes = engine.get_tensor_shape_ptr(tname);
